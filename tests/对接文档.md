...
sequenceDiagram
    ...
    participant YieldPayer as 收益来源方(募资发起人或业务收入)
    ...
    %% 2) 投资
    Investor ->> invest: transfer(goal_asset, plan:plan_id)
    invest ->> invest: _process_investment
    invest ->> invest: issue receipt voucher
    invest ->> stake: transfer receipt (stake:plan_id:investor)
    Note right of stake:
        on_transfer_rwafi
        记录投资人 stake 凭证

    %% 3) 担保
    Guarantor ->> guaranty: transfer(goal_asset, guaranty:plan_id)
    guaranty ->> guaranty: update guarantee stats
    Note right of guaranty:
        条件:
        now < end_time
        status 为 PENDING 或 RAISEACTIVE 或 SUCCESS
    ...
    %% 5) 收益进入 yield.rwa
    YieldPayer ->> yield: transfer(goal_asset, plan:plan_id)
    yield ->> yield: on_transfer
    yield ->> yield: perform distribution

    %% 5.1 收益拆分
    yield ->> stake: transfer investor reward (reward:plan_id)
    yield ->> guaranty: transfer guarantor reward (reward:plan_id)
    ...
    yield ->> yield: log monthly yield

    %% 6) 回购（任何人可触发）
    Executor ->> yield: buyback(plan_id)
    yield ->> yield: swap buyback asset via AMM
    Note right of yield:
        使用 plan_buyback 累积余额
        不影响投资与担保收益

    %% 7) 年度兜底
    Executor ->> guaranty: guarantpay(plan_id)
    guaranty ->> guaranty: calculate yearly gap
    guaranty ->> guaranty: deduct from guarantors
    guaranty ->> stake: transfer补偿资金
    Note right of guaranty:
        SUCCESS 状态
    ...
    Note right of guaranty:
        FAILED 或 CANCELLED 全额赎回
        SUCCESS 未到期 coverage >= 50% 可部分
        COMPLETED 或到期 全额赎回
```