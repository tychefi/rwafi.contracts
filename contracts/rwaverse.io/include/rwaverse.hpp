...
enum class rwaverse_type: uint8_t {
   RANDOM       = 0,
   MEAN         = 1,
   DID_RANDOM   = 10,
   DID_MEAN     = 11
...
public:
    ...
    rwaverse(eosio::name receiver, eosio::name code, datastream<const char*> ds):
        _db(_self),
        _db_stake(_self),
    ...
    ACTION setwhitelist(const name& account, const bool& enabled);
    ...
    using liquidity_action          = eosio::action_wrapper<"liquidity"_n, &rwaverse::liquidity>;
    ...
    using notify_action             = eosio::action_wrapper<"notify"_n, &rwaverse::notify>;
