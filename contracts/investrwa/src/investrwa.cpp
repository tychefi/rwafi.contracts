...
#include "stakerwa/stakerwadb.hpp"
#include <algorithm>
#include <chrono>
#include <eosio/transaction.hpp>
#include <eosio/crypto.hpp>
#include "guarantyrwa/guarantyrwadb.hpp"
...
void investrwa::_process_investment(const name& from,const name&,const asset& quantity,const string&,fundplan_t& plan) {
    ...
    ISSUE(plan.receipt_asset_contract, from, asset(accepted.amount, plan.receipt_symbol),
                  "plan:" + to_string(plan.id));

    if (refund.amount > 0)
        TRANSFER(plan.goal_asset_contract, from, refund, "refund: exceed hard cap " + to_string(plan.id));
...
void investrwa::_process_refund(const name& from,const name&,const asset& quantity,const string&,fundplan_t& plan) {
    ...
    BURN(plan.receipt_asset_contract, from, quantity, "burn receipt for refund: " + to_string(plan.id));
    TRANSFER(plan.goal_asset_contract, from, refund_amount, "refund for plan: " + to_string(plan.id));
...
void investrwa::on_transfer(const name& from, const name& to, const asset& quantity, const string& memo) {
    ...
    if (bank == SING_BANK && quantity.symbol == plan.goal_quantity.symbol) {
        _process_investment(from, to, quantity, memo, plan);
        return;

    } else if (bank == RECEIPT_BANK && quantity.symbol == plan.receipt_symbol) {
        _process_refund(from, to, quantity, memo, plan);
        return;

    } else {
        CHECKC(false, err::SYMBOL_MISMATCH,
               "symbol mismatch, expected: " + plan.goal_quantity.symbol.code().to_string() +
               " or " + plan.receipt_symbol.code().to_string() +
               ", actual: " + quantity.symbol.code().to_string());
    }
}
...
void investrwa::createplan(const name& creator,
    ...
    if (statstable.find(receipt_quantity_per_unit.symbol.code().raw()) == statstable.end())
        CREATE(RECEIPT_TOKEN_BANK,_self,asset(goal_quantity.amount, receipt_quantity_per_unit.symbol));
