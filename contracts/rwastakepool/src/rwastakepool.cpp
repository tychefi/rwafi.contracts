#include "rwastakepool.hpp"
#include "flon/flon.token.hpp"
#include "rwaverse.io/rwaversedb.hpp"

namespace rwafi {

uint64_t rwastakepool::calc_reward_per_share_delta(const asset& rewards,const asset& total_staked) {
...
asset rwastakepool::calc_user_reward(const asset& staked,const uint64_t& reward_per_share_delta,const symbol& reward_symbol) {
...
void rwastakepool::notify(const name& contract,
                            const name& from,
                            const name& to,
...
void rwastakepool::init(const name& admin, const name& rwaverse_contract) {
    require_auth(get_self());

    CHECKC(is_account(admin), err::ACCOUNT_INVALID, "invalid admin");
    CHECKC(is_account(rwaverse_contract), err::ACCOUNT_INVALID, "invalid invest contract");

    _gstate.admin              = admin;
    _gstate.rwaverse_contract = rwaverse_contract;
}

void rwastakepool::addplan(const uint64_t& plan_id,const symbol& receipt_symbol,const name& reward_token_contract,const symbol& reward_symbol) {
    CHECKC(has_auth(_gstate.admin) || has_auth(_gstate.rwaverse_contract),err::NO_AUTH,"missing required auth");

    // ------------------------------------------------------------
    // 1) 校验 rwaverse.io 中的 fundplan
    // ------------------------------------------------------------
    CHECKC(reward_symbol == SING_SYM, err::SYMBOL_MISMATCH, "reward symbol must be SING");
    fundplan_t::idx_t fundplans(_gstate.rwaverse_contract, _gstate.rwaverse_contract.value);

    auto fund_itr = fundplans.find(plan_id);
    CHECKC(fund_itr != fundplans.end(),err::RECORD_NOT_FOUND, "fundplan not found in rwaverse");
    CHECKC(fund_itr->receipt_symbol == receipt_symbol, err::SYMBOL_MISMATCH, "receipt symbol mismatch with fundplan");

    // ------------------------------------------------------------
    // 2) 检查 rwastakepool 是否已存在
    // ------------------------------------------------------------
...
void rwastakepool::delplan(const uint64_t& plan_id) {
...
void rwastakepool::claim(const name& owner, const uint64_t& plan_id) {
    require_auth(owner);
    _claim(owner, plan_id,true);
}


void rwastakepool::_claim(const name& owner, const uint64_t& plan_id,bool strict) {
    ...
    //
    rwastakepool::notify_action act{ get_self(), { {get_self(), "active"_n} } };
...
// --- 质押凭证 ---
void rwastakepool::on_transfer_rwafi(const name& from,const name& to,const asset& quantity,const string& memo) {
    if (from == get_self() || to != get_self()) return;

    CHECKC(quantity.amount > 0, err::NOT_POSITIVE, "must transfer positive amount");
    // CHECKC(from == INVEST_POOL, err::NO_AUTH, "only rwaverse.io contract can stake receipts");
...
// --- 管理员充值奖励 ---
void rwastakepool::on_transfer_reward(const name& from, const name& to, const asset& quantity, const string& memo) {
    if (from == get_self() || to != get_self()) return;
    CHECKC(quantity.amount > 0, err::NOT_POSITIVE, "must transfer positive amount");
...
//提取凭证
void rwastakepool::unstake(const name& owner, const uint64_t& plan_id, const asset& receipt_quantity) {
    require_auth(owner);
...
void rwastakepool::batchunstake(const uint64_t& plan_id) {
    // ===  只有 rwaverse.io 主合约可触发退款流程 ===
    require_auth(INVEST_POOL);

    // ===  查询 rwaverse.io 上的 fundplan 状态 ===
    fundplan_t::idx_t fundplans(_gstate.rwaverse_contract, _gstate.rwaverse_contract.value);
    ...
    for (auto itr = stakers.begin(); itr != stakers.end();) {
        ...
        // === 资金流：rwastakepool → rwaverse.io（退回 receipt token）===
        TRANSFER("rwa.vtoken"_n, INVEST_POOL, refund_amount, memo);
...
void rwastakepool::_on_stake(const name& from,const asset& quantity,const uint64_t& plan_id,const stake_plan_t& plan) {
...
void rwastakepool::_on_reward_in(const name& from, const asset& quantity, const uint64_t& plan_id) {
