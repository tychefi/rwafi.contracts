...
#define TBL struct [[eosio::table, eosio::contract("rwastakepool")]]
#define NTBL(name) struct [[eosio::table(name), eosio::contract("rwastakepool")]]
...
NTBL("global") global_t {
    name                admin               ;                       // 管理员账户
    name                rwaverse_contract  ;                       // RWA 投资主合约

    EOSLIB_SERIALIZE(global_t,
        (admin)(rwaverse_contract))
};
...
struct stake_reward_st {
    ...
    name            reward_token_contract = SING_BANK;              // 发奖代币合约
    symbol          reward_symbol         = SING_SYM;               // 奖励代币符号
};

//Scope: _self
TBL stake_plan_t {
    uint64_t            plan_id;                                    // 主键: 对应 rwaverse.io 的 fundplan.id
