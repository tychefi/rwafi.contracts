#include "rwaguarapool.hpp"
#include "rwayieldpool/rwayieldpool.hpp"
...
namespace {
void refresh_plan_status(const name& rwaverse_contract, const name& submitter, uint64_t plan_id) {
    eosio::action(
...
// 当前年月（YYYYMM）
uint64_t rwaguarapool::_current_period_yyyymm() {
...
// ------------------------------------------------------------
asset rwaguarapool::_yearly_guarantee_principal(const fundplan_t& plan) {
...
// ------------------------------------------------------------
guarantor_stake_t::idx_t rwaguarapool::_get_stake_tbl(uint64_t plan_id) const {
...
// 从 wasm_db 读取担保池统计
// ------------------------------------------------------------
guaranty_stats_t rwaguarapool::_get_stats_or_fail(uint64_t plan_id) {
...
// 读取单个担保人记录（按值返回）
// ------------------------------------------------------------
guarantor_stake_t  rwaguarapool::_get_stake_or_fail(uint64_t plan_id, const name& guarantor) {
...
// ------------------------------------------------------------
uint32_t rwaguarapool::_years_passed(const fundplan_t& plan) const {
...
// ------------------------------------------------------------
// 投资人累计收益（唯一权威口径）
// 来源：rwastakepool::stake_plan.reward_state.total_rewards
// ------------------------------------------------------------
int64_t rwaguarapool::_calc_investor_yield_sum(uint64_t plan_id,const symbol& sym) const {
...
rwaguarapool::CoverageInfo rwaguarapool::_calc_coverage(const fundplan_t& plan,const guaranty_stats_t& stats,const symbol& sym) const {
    CoverageInfo info{};
...
void rwaguarapool::init(const name& admin) {
...
void rwaguarapool::on_transfer(const name& from, const name& to, const asset& quantity, const string& memo){
...
void rwaguarapool::_handle_guaranty_transfer(const name& from,const fundplan_t& plan,const asset& quantity) {
...
void rwaguarapool::_handle_reward_transfer(const fundplan_t& plan, const asset&      quantity) {
...
void rwaguarapool::guarantpay(const name& submitter, const uint64_t& plan_id){
...
void rwaguarapool::redeem(const name& guarantor, const uint64_t& plan_id, const asset& quantity){
...
void rwaguarapool::_redeem_failed_project(const name& guarantor,const fundplan_t& plan,guaranty_stats_t& stats,const asset& quantity){
...
void rwaguarapool::_redeem_in_progress(const name& guarantor,const fundplan_t& plan,guaranty_stats_t& stats,const asset& quantity){
...
void rwaguarapool::_redeem_project_end(const name& guarantor,const fundplan_t& plan,guaranty_stats_t& stats,const asset& quantity){
...
void rwaguarapool::_deduct_from_guarantors(uint64_t plan_id,const asset& pay) {
...
redeem_cap_t rwaguarapool::_calc_guarantor_redeemable_in_progress(uint64_t plan_id, const fundplan_t& plan, guaranty_stats_t& stats, const symbol& sym,const guarantor_stake_t& user_row,int64_t redeem_q) const{
    redeem_cap_t cap;
