...
void guarantyrwa::redeem(const name& guarantor, const uint64_t& plan_id, const asset& quantity) {
    ...
    if (project_failed) {
        ...
        _redeem_project_end(guarantor, plan, stats, conf, quantity);
    } else {
        _redeem_in_progress(guarantor, plan, stats, conf, quantity);
    }
}
...
void guarantyrwa::_redeem_failed_project(const name& guarantor,
    ...
    CHECKC(quantity <= available, err::QUANTITY_INSUFFICIENT, "redeem exceeds available funds");

    _do_redeem(guarantor, plan, stats, quantity,
                 "redeem due to project failure, plan:" + std::to_string(plan.id));
}
...
void guarantyrwa::_redeem_in_progress(const name& guarantor,
    ...
    CHECKC(quantity <= unlocked_asset,
           err::QUANTITY_INSUFFICIENT,
           "redeem amount " + quantity.to_string() +
           " exceeds unlocked balance " + unlocked_asset.to_string());

    _do_redeem(guarantor, plan, stats, quantity,
                 "redeem unlocked guarantee funds ,plan:" + std::to_string(plan.id));
}
...
void guarantyrwa::_redeem_project_end(const name& guarantor,
    ...
    CHECKC(unlocked_asset.amount > 0, err::QUANTITY_INSUFFICIENT, "no remaining funds to redeem");
    ...
    _do_redeem(guarantor, plan, stats, quantity,
                 "redeem remaining guarantee funds (after settlement) for plan:" + std::to_string(plan.id));
}
...
void guarantyrwa::_do_redeem(const name& guarantor,
    ...
    CHECKC(itr->total_funds >= quantity,
           ...
           " — tried to redeem " + quantity.to_string() +
